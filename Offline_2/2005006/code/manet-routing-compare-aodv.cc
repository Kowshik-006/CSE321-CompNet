...
std::string m_CSVfileName{"manet-routing.aodv.output.csv"}; //!< CSV filename.
...
void CalculateMetrics(Ptr<FlowMonitor> monitor, Ptr<Ipv4FlowClassifier> classifier) {
    ...
    std::ofstream out(m_CSVfileName, std::ios::app);
    if(m_CSVfileName == "node_num_aodv.csv")
        out << nWifis << ",";
    else if(m_CSVfileName == "node_speed_aodv.csv")
        out << nodeSpeed << ",";
    else if(m_CSVfileName == "packet_rate_aodv.csv")
