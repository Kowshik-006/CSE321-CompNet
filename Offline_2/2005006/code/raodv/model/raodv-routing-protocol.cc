...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        // Dynamically finding origin of current interface
        rreqHeader.SetOrigin(iface.GetLocal());
        // Insert the newly built packet into own cache
        // to avoid duplicate reception
...
RoutingProtocol::RecvRAodv(Ptr<Socket> socket)
{
    ...
    // sit in a loop until packet comes
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    // RAODV -> Here the reply will be broadcasted instead of unnicasting
    if (IsMyOwnAddress(rreqHeader.GetDst()))
    {
        m_routingTable.LookupRoute(origin, toOrigin);
        NS_LOG_DEBUG("Send reply since I am the destination");
        SendReply(rreqHeader, toOrigin);
        // SendReply_Broadcast(rreqHeader.GetDst(),rreqHeader.GetOrigin());
        return;
    }
    /*
...
{
    NS_LOG_FUNCTION(this);
    RevRreqHeader rev_rreqHeader;
    p->RemoveHeader(rev_rreqHeader);
    ...
    uint32_t id = rev_rreqHeader.GetId();
    Ipv4Address origin = rev_rreqHeader.GetOrigin();
    ...
    // Increment REV_RREQ hop count
    uint8_t hop = rev_rreqHeader.GetHopCount() + 1;
    rev_rreqHeader.SetHopCount(hop);
    ...
    if (!m_routingTable.LookupRoute(origin, toOrigin))
    {
        ...
        RoutingTableEntry newEntry(
            ...
            /*dst=*/origin,
            /*vSeqNo=*/true,
            /*seqNo=*/rev_rreqHeader.GetDstSeqno(),
            /*iface=*/m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
            /*hops=*/hop,
            /*nextHop=*/src,
            /*lifetime=*/Time(2 * m_netTraversalTime - 2 * hop * m_nodeTraversalTime));
    ...
    else
    {
        if (toOrigin.GetValidSeqNo())
        {
            if (int32_t(rev_rreqHeader.GetDstSeqno()) - int32_t(toOrigin.GetSeqNo()) > 0)
            {
                toOrigin.SetSeqNo(rev_rreqHeader.GetDstSeqno());
            }
        }
        else
        {
    ...
    else
    {
        toNeighbor.SetLifeTime(m_activeRouteTimeout);
        toNeighbor.SetValidSeqNo(false);
        toNeighbor.SetSeqNo(rev_rreqHeader.GetDstSeqno());
        toNeighbor.SetFlag(VALID);
        toNeighbor.SetOutputDevice(m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver)));
        toNeighbor.SetInterface(m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0));
        toNeighbor.SetHop(1);
    ...
    NS_LOG_LOGIC(receiver << " receive REV_RREQ with hop count "
                          << static_cast<uint32_t>(rev_rreqHeader.GetHopCount()) << " ID "
                          << rev_rreqHeader.GetId() << " to destination " << rev_rreqHeader.GetDst());

    Ipv4Address dst = rev_rreqHeader.GetDst();
    Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
    RoutingTableEntry newEntry(
        /*dev=*/dev,
        /*dst=*/dst,
        ...
        /*hops=*/hop,
        /*nextHop=*/src,
        /*lifetime=*/rev_rreqHeader.GetLifeTime());
    RoutingTableEntry toDst;
    if (m_routingTable.LookupRoute(dst, toDst))
    {
        ...
        if (
            ...
            ((int32_t(rev_rreqHeader.GetDstSeqno()) - int32_t(toDst.GetSeqNo())) > 0) ||

            // (iii) the sequence numbers are the same, but the route is marked as inactive.
            (rev_rreqHeader.GetDstSeqno() == toDst.GetSeqNo() && toDst.GetFlag() != VALID) ||

            // (iv) the sequence numbers are the same, and the New Hop Count is smaller than the
            // hop count in route table entry.
            (rev_rreqHeader.GetDstSeqno() == toDst.GetSeqNo() && hop < toDst.GetHop()))
        {
            m_routingTable.Update(newEntry);
        }
    }
    ...
    // if (rev_rreqHeader.GetAckRequired())
    // {
    //     SendReplyAck(src);
    //     rev_rreqHeader.SetAckRequired(false);
    // }
    // NS_LOG_LOGIC("receiver " << receiver << " origin " << rrepHeader.GetOrigin());
    
    // The moment when the origin receives the reply
    if (IsMyOwnAddress(rev_rreqHeader.GetOrigin()))
    {
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    RevRreqHeader rev_rreqheader = RevRreqHeader();
    rev_rreqheader.SetHopCount(0);
    rev_rreqheader.SetDst(rreqHeader.GetDst());
    rev_rreqheader.SetDstSeqno(m_seqNo);
    rev_rreqheader.SetOrigin(toOrigin.GetDestination());
    rev_rreqheader.SetLifeTime(m_myRouteTimeout);
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        // Dynamically finding origin of current interface
        rev_rreqheader.SetDst(iface.GetLocal());
        // Insert the newly built packet into own cache
        // to avoid duplicate reception
        m_rreqIdCache.IsDuplicate(iface.GetLocal(), m_requestId);
    ...
    // ScheduleRreqRetry(rreqHeader.GetOrigin());
}
                                             ...
                                             bool gratRep)
{
    ...
    // precursor -> whoch nodes depend on me for finding a route
